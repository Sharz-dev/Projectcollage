...
class TestRegister(unittest.TestCase):
    ...
    def test_register_success(self):
        with app.app_context():
            ...
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.get_json(), 'User added successfully!')
    ...
    def test_register_existing_user(self):
        ...
        with app.app_context():
            ...
            self.assertEqual(response.get_json(), 'User already exist !')
    ...
    def test_register_missing_params(self):
        with app.app_context():
            ...
            self.assertEqual(response.get_json(), '{"message": " Some Columns are missing or Mispelled the Column name"}')
...
class TestUpdateMovie(unittest.TestCase):
    def setUp(self):
        ...
        self.language = "English"
        self.director = "Frank "
        self.request = Mock(method='PUT')

    def test_update_movie_success(self):
        with app.app_context():
            response = updateMovie(self.movieid)
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.get_json(), {"message": "movie updated successfully!"})

    def test_update_movie_missing_params(self):
        with app.app_context():
            self.request.json = {
                "moviename": "",
                "moviegenre": "",
            ...
            response = updateMovie(self.movieid)
            self.assertEqual(response.status_code, 400)
            self.assertEqual(response.get_json(), {"message": "Bad Request - Missing Required Fields"})
